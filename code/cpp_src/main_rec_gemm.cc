...
void naive_range(int K, int numTiles,
		 ...
		 T2 & romState)
{
  Kokkos::View<double**>   fomState("fomY", phiRowsPerTile*numTiles, numApps);
  ...
  for (int count=0; count<loopCount; ++count)
    {
      Kokkos::parallel_for(numTiles,
			   KOKKOS_LAMBDA(const int i)
			   {
			     ...
			     auto myRomState = Kokkos::subview(romState, r1, Kokkos::ALL());
      ...
      Kokkos::parallel_for(policy,
			   KOKKOS_LAMBDA(const member_type & member)
			   {
			     ...
			     KokkosBatched::TeamVectorGemm<
			       member_type, notr, notr, alg>::invoke(member, 1.0, myPhi, myRomState,
							     0.0, myFomState);
			   });
      Kokkos::fence();
    }
	    ...
	    T2 & romState)
{
  ...
  typedef Kokkos::DefaultExecutionSpace SpT;
  Kokkos::TeamPolicy<SpT> policy(q*numTiles, Kokkos::AUTO());
  typedef Kokkos::TeamPolicy<SpT>::member_type member_type;
  ...
  for (int count=0; count<loopCount; ++count)
	...
	Kokkos::parallel_for(policy,
			     KOKKOS_LAMBDA(const member_type & member)
			     {
            const std::size_t bId = member.league_rank();
	    ...
	    KokkosBatched::TeamVectorGemm<
	      member_type, notr, notr, alg>::invoke(member, 1.0, myPhi,
						    myRomState,
						    beta, myFomState);
	  });
	Kokkos::fence();
...
int main(int argc, char *argv[])
{
  ...
  app.add_option("-a", numApps);
  CLI11_PARSE(app, argc, argv);

  Kokkos::initialize();
  {
    Kokkos::View<double***> phis("phis", numTiles, phiRowsPerTile, K);
    Kokkos::View<double**>   romState("romState", numTiles*K, numApps);
    ...
    naive_range(K, numTiles, phiRowsPerTile, loopCount, numApps, phis, romState);
    naive_team(K, numTiles, phiRowsPerTile, loopCount, numApps, phis, romState);
    nested(K, numTiles, phiRowsPerTile, loopCount, numApps,
